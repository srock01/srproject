...
export default function NetworkView ({navigation, route}) {
  const [clothes, setClothes] = useState([]);
  
  const myContext = useContext(Context);
  const email = myContext.email;  
  const org = myContext.nwork;
  console.log(org);
  function fetchBlogs1(name,myContext)  {
    console.log(name+'fjk');
    myContext.setUL(name);
...
useEffect(() => {
  ...
  return (
    <View style={{flex:1, backgroundColor:"#1C4BA5"}}>
      ...
      <Text style={styles.buttonTxt1}>{org} LEAGUES</Text>
        <FlatList
          data={clothes}
            ...
            <TouchableOpacity style={styles.items}
              onPress={() => fetchBlogs1(item.id,myContext)} >
               <View style={styles.budgetTagsContainer}>
                ...
                <Text style={styles.name}>Sport: {item.sport}</Text>
              </View>
            </TouchableOpacity>
          </View>)}/>
      </View>
